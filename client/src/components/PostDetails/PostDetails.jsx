						...
						<Grid className={classes.recommendedPosts}>
							{recommendedPosts.map(({ title, message, name, likes, selectedFile, _id }) => (
								<div style={{ margin: '20px', cursor: 'pointer', maxWidth: '200px' }} onClick={() => openPost(_id)} key={_id}>
									...
									<Typography gutterBottom variant="subtitle2" sx={{ color: 'black' }}>
										By: <i>{name}</i>
									</Typography>
									<img src={selectedFile} alt={'selected File'} width="200px" />
									<Typography gutterBottom variant="subtitle2" textAlign="justify" sx={{ color: 'black' }}>{`${message.slice(0, 200)} ${message.length > 200 ? '...' : ''}`}</Typography>
									<Typography gutterBottom variant="subtitle1" sx={{ color: 'black' }}>
										Likes: {likes.length}
									</Typography>
								</div>
							))}
